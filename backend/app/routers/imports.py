...
class WebImportRequest(BaseModel):
    ...
    max_products: int = Field(default=20, ge=1, le=20)
...
class BulkWebImportRequest(BaseModel):
    ...
    max_products: int = Field(default=500, ge=1, le=1000)


@router.post("/web", dependencies=[Depends(verify_admin_key)])
def import_web_products(request: WebImportRequest) -> dict[str, object]:
    # 최대 20개인 일반 수집은 기존 API 호환성을 위해 동기 처리합니다.
...
def start_bulk_import(request: BulkWebImportRequest) -> dict[str, object]:
...
def read_import_job(job_id: str) -> dict[str, object]:
